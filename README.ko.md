...
같은 일, 같은 날, 같은 머신이다. 차이는 오로지 에이전트가 도는 동안 오케스트레이터가 조용히 있었는가
하나다.
...
flowchart LR
    ...
    B --> C["3. 브리프를 파일로<br/>prompt는 경로만"]
    C --> C2["3a. 투입 직후 한 턴<br/>base 맞나? 브리프·env 복사"]
    C2 --> D["4. 침묵<br/>0턴, 무료"]
    D --> E["5. 수확<br/>rebase → 충돌 → 게이트 → 푸시"]
    E --> F["6. 아직 도나?<br/>제거 «전»에 확인"]
...
필요한 턴에만 읽는다. 투입 턴이 여는 RUNBOOK은 «투입 직후 점검» 한 절뿐이고, 수확 절차는 수확할 때 읽는다.
